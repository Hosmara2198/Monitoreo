    ...
    public function sendMessageReport($sendMessage, $status){
        ...
        if (isset($data)) {
            if ($data->status == false) {
                ...
                if (isset($data)) {
                    if ($data->status == true) {
                        BitacoraMessageFb::where('id', $sendMessage->id)->update(['error' => $data->status, 'status' => $status]);
                    } else if ($data->status == "successfully queued") {
                        BitacoraMessageFb::where('id', $sendMessage->id)->update(['error' => $data->status, 'status' => $status]);
                    } else {
                        BitacoraMessageFb::where('id', $sendMessage->id)->update(['error' => $data->message, 'status' => 0]);
                    }
                }
            } else if ($data->status == "successfully queued") {
                BitacoraMessageFb::where('id', $sendMessage->id)->update(['error' => $data->status, 'status' => $status]);
            } else {
                BitacoraMessageFb::where('id', $sendMessage->id)->update(['error' => $data->status, 'status' => 0]);
            }
        } else {
            $send = curl_init();
            curl_setopt_array($send, array(
            ...
            if (isset($data)) {
                ...
                BitacoraMessageFb::where('id', $sendMessage->id)->update(['error' => $result, 'status' => 0]);
            }
        }
    }
}
